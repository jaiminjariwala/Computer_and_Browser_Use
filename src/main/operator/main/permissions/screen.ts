/**
 * Screen Recording permission — VENDORED from Codex Lite `permissions.ts`
 * (Task 2).
 *
 * Reuse rule (Req 19): a one-time COPY into the Codex Lite tree; it does not
 * import from or modify the `computer-or-browser-use` project. Codex Lite owns and
...
export function mapStatusToResult(
    ...
    const message = previouslyGranted
        ? 'Screen Recording permission was turned off, so Codex Lite cannot capture your screen. Re-enable it in System Settings to continue.'
        : 'Codex Lite needs Screen Recording permission to capture your screen. Enable it in System Settings to continue.'
