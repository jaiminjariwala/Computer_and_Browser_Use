...
/**
 * OpenAI-compatible gateway client — VENDORED from Codex Lite `ai.ts` (Task 2).
 *
 * STATUS (as of the provider-layer refactor): LEGACY / currently unused. Nothing
 * in Codex Lite imports this module — the live reasoning path is
 ...
 *
 * Reuse rule (Req 19): this is a one-time COPY into the Codex Lite tree
 * (`src/main/providers/`); it does not import from or modify the `computer-or-browser-use`
 * project. Codex Lite now owns and evolves this copy. It is the base for the
 ...
 * SDK client are reused as-is here; provider generalization happens in Task 6.
 *
 * This module owns two responsibilities as vendored:
 *  1. Pure, testable assembly of chat-completion messages from a
 *     `SessionContext` / `Turn[]` (system contract, running summary, recent
 *     turns, and image captures as `image_url` content parts).
 *  2. A thin client that points the `openai` SDK at the configured gateway
 *     (`baseURL` + credentials) and runs `complete` / `summarize`.
 */

/**
 * System behavior contract sent as the first message of every `complete`
 * request. NOTE: this is Codex Lite's advice-only contract as vendored; the
 * Codex Lite reasoning contract (computer/task_complete/request_help tools,
 * one Action per Reasoning_Step) is built in Task 6.4.
 */
export const SYSTEM_PROMPT = [
    'You are Codex Lite, an advice-only, screen-aware desktop co-pilot that helps the user make progress in whatever application they are using (browsers, design tools, editors, spreadsheets, and so on).',
    '',
    'Inferring intent:',
 ...
 * Codex Lite does not use audio, but the client shape is retained.
 */
 ...
 * The merge logic that preserves intent + completed steps lives in
 * {@link mergeSummary}.
 */
...
/** The conversation interface used by the rest of the app. */
export interface AIClient {
    /** Produce guidance/next-step from the current session context. */
 ...
 * message. Always produces a string so every request carries session context,
...
/** Build the `image_url` content part for a capture. */
...
/**
 * Map a single {@link Turn} to a chat-completion message.
 *
...
/**
 * Assemble the full message list for a `complete` request:
 ...
 * the inferred intent is retained when the model returns nothing useful, and
 * completed steps are unioned so existing steps are never dropped.
 * `updatedThroughTurnId` advances to the last folded turn.
 */
 ...
 * off `.glassError` and surface it without re-inventing the mapping. The
 * original cause is preserved on `.cause` for logging/debugging. (Superseded by
 * `OperatorError` in later tasks.)
 */
export class GlassErrorException extends Error {
    /** The typed, user-facing failure to surface. */
 ...
 * route the user to Settings; recovery is to enter credentials.
 */
...
export interface AIClientOptions {
    ...
    /** Resolve the fallback API key, or null (local providers need none). */
...
export class GatewayAIClient implements AIClient {
     ...
     * {@link GlassError} when no API key is configured.
     */
     ...
     * An already-typed {@link GlassErrorException} passes through unchanged so it
     * is not re-wrapped as a gateway failure.
     */
     ...
     * Vendored as-is; Codex Lite does not use audio in v1. Uses the PRIMARY
