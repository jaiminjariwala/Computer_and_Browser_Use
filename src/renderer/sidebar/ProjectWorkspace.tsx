...
export function ProjectWorkspace({ visible, artifact, onClose, width, onResize, browserObscured = false, tabHost }: {
    ...
    return <aside className="project-workspace" aria-label="Project workspace" style={{ display: visible ? 'flex' : 'none', width, maxWidth: 'calc(100vw - 300px)' }}>
        ...
        {tab?.kind !== 'browser' && <div className="project-tree-toggle"><button type="button" aria-label="Toggle files tree" aria-expanded={treeOpen} title={treeOpen ? 'Collapse files tree' : 'Expand files tree'} onClick={() => setTreeOpen(value => !value)}><WorkspaceIcon name="files" /></button></div>}
